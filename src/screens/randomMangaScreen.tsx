...
import { getRandomManga, type Manga } from '../assets/API/jikan';

const RandomMangaScreen: React.FC = () => {
	const [wallpaper, _setWallpaper] = useState<string>(() => getRandomWallpaper('randomManga'));
	...
	const isAdultManga = (m: Manga | null): boolean => {
		const names = [
			...(m?.genres ?? []),
			...(m?.explicit_genres ?? []),
			...(m?.themes ?? []),
		].map((g) => (g?.name || '').toLowerCase());
		return names.includes('hentai') || names.includes('erotica');
	};
	...
	const draw = async () => {
		...
		try {
			let picked: Manga | null = null;
			const attempts = allowAdult ? 1 : 8;
			for (let i = 0; i < attempts; i++) {
				const resp = await getRandomManga();
				if (allowAdult || !isAdultManga(resp.data)) {
					picked = resp.data;
					break;
				}
			}
			...
			if (!picked) {
				setAlert({ visible: true, type: 'warning', message: 'Não foi possível encontrar um mangá SFW. Tente novamente.' });
				setPhase('idle');
				return;
			}
			setSelected(picked);
			setPhase('reveal');
			setAlert({ visible: true, type: 'success', message: `Mangá sorteado: ${picked.title}` });
			if (apiModalEnabled) setLastApiPayload({ endpoint: 'random/manga', allowAdult, response: picked });
		} catch (e) {
			setAlert({ visible: true, type: 'error', message: 'Falha ao sortear mangá. Tente novamente.' });
			setPhase('idle');
		} finally {
			setIsLoading(false);
		}
	...
	const malUrl = selected ? `https://myanimelist.net/manga/${selected.mal_id}` : undefined;
		...
		<BackgroundImage src={wallpaper} className="relative w-full min-h-screen bg-cover bg-no-repeat bg-center bg-fixed">
			...
			<div className="relative z-10 w-full min-h-screen max-w-[92vw] 2xl:max-w-[1900px] mx-auto px-4 sm:px-6 lg:px-12">
				...
				<Box className="bg-black/40 rounded-lg backdrop-blur-sm border border-white/20 shadow-lg w-full max-w-none mx-auto p-4 sm:p-6 md:p-8 mt-6 sm:mt-10 lg:mt-12 mb-16 h-[560px] md:h-[640px] lg:h-[720px]">
								...
								<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch h-full">
									...
									<div className="lg:col-span-2 flex flex-col gap-4 min-h-0 h-full">
										...
										<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-(--colorTextWhite)">
											<Stat label="Capítulos" value={selected.chapters ?? '-'} />
					...
					{/* Botão inicial removido conforme pedido */}
				</Box>
			</div>
		</BackgroundImage>
	);
...
export default RandomMangaScreen;
