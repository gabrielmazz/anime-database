...
export type SelectionLayout = {
  columns: 1 | 2;
  cardMinHeight: number; // in px
  titleFontSize: string; // CSS clamp string
  cardTitleFontSize: string; // CSS clamp string
};
 ...
 */
export function getSelectionLayoutFor(width: number, _height?: number): SelectionLayout {
  const columns: 1 | 2 = width < 768 ? 1 : 2; // md breakpoint ~768px

  let cardMinHeight = 240;
  if (width < 480) cardMinHeight = 160; // mobile pequeno
  else if (width < 768) cardMinHeight = 200; // mobile grande / tablet vertical
